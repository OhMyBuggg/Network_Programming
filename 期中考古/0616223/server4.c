    ...
    for(;;){
        ...
        for(c=0; c<=maxClient; c++){
            ...
            if(FD_ISSET(client[c], &rset)){
                ...
                ssize_t len;
                int now;
                for(d=0; d<tablenum; d++){
                    if(table[d].connect == connfd){
                        now = table[d].data;
                        break;
                    }
                }
                if(d >= tablenum) now = connfd;
                len = read(connfd, &buffer, MAXLINE);
                strcpy(history[now][i[now]], buffer);
                ...
                if(strcmp(cmd, "DEPOSIT") == 0){
                    ...
                    if(sum[now] - num < 0){
                        ...
                        write(connfd, message, sizeof(message));
                        char temp[MAXLINE];
                        strcpy(temp, history[now][i[now]]);
                        if(temp[strlen(temp)-1] == '\n')temp[strlen(temp)-1] = ' ';
                        sprintf(failedcmd, "%s(FAILED)\n", temp);
                        strcpy(history[now][i[now]], failedcmd);
                        i[now]++;
                        continue;
                    }
                    sum[now] -= num;
                    ...
                    if(i[now] == 0){
                        ...
                        for(j=0; j<i[now]; j++){
                            sprintf(message, "### %s", history[now][j]);
                            write(connfd, message, sizeof(message));
                        }
                    }
                    sprintf(message, "$$$ BALANCE: %d NTD\n", sum[now]);
                    ...
                    for(k=0; k<tablenum; k++){
                        if(strcmp(arg, table[k].clientID) == 0){
                            table[k].connect = connfd;
                            for(int y=0; y<tablenum; y++){
                                if(table[y].connect == table[k].connect && y != k){
                                    table[y].connect = -1;
                                }
                            }
                            break;
                    ...
                    if(k == tablenum){
                        strcpy(table[k].clientID, arg);
                        table[k].connect = connfd;
                        table[k].data = tablenum;
                        for(int y=0; y<tablenum; y++){
                                if(table[y].connect == table[k].connect && y != k){
                                    table[y].connect = -1;
                                }
